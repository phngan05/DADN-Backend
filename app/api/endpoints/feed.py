...
def _validate_feed_category(category: str):
    if category not in VALID_FEED_CATEGORIES:
        raise HTTPException(status_code=400, detail="Invalid category")


@router.get("")
def get_feeds(user_id: str = Depends(get_current_user_id)):
    try:
        print(user_id)
        query = supabase_client.table("FEED")\
        .select("feed_id, feed_key, category, ADAFRUIT_SERVER!inner(server_id, username)")\
        .eq("ADAFRUIT_SERVER.user_id", user_id)\
            
        response = query.execute()

        # Ignore legacy feed categories that are no longer supported.
        return [
            feed for feed in response.data
            if feed.get("category") in VALID_FEED_CATEGORIES
        ]
...
def update_feed(feed: FeedUpdate):
    try:
        ...
        _validate_feed_category(feed.category)
